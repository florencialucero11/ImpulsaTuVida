  ...
  const items = [
    {
      ...
      texto: 'Recibí planes 100% personalizados según tu meta personal. Sin plantillas, sin fórmulas mágicas.',
    },
    {
      icono: <Zap size={32} className="text-emerald-400" />,
      titulo: 'Respuestas en segundos',
      texto: 'Ingresás tu objetivo y en un clic obtenés tu plan de acción motivador.',
    },
    {
      icono: <Globe size={32} className="text-emerald-400" />,
      titulo: 'Para todos y todas',
      texto: 'Funciona para cualquier tipo de persona y objetivo: salud, carrera, finanzas, emociones o estudio.',
    },
    {
      icono: <Smartphone size={32} className="text-emerald-400" />,
      titulo: 'Accesible desde cualquier lugar',
      texto: 'Diseño adaptado a dispositivos móviles. Llevá tu progreso donde vayas.',
    },
  ];
    ...
    <section className="w-full max-w-6xl mx-auto mt-24 px-6">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {items.map(({ icono, titulo, texto }, i) => (
          <div
            key={i}
            className="flex items-start gap-4 bg-white/10 border border-white/10 p-6 rounded-3xl shadow-xl backdrop-blur-xl hover:scale-[1.02] transition-transform"
          >
            <div>{icono}</div>
            <div>
